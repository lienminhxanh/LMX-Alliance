  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-[#1F2937]" style={{ fontFamily: 'var(--font-display)' }}>News</h1>
        <Link href="/admin/news/new"><Button size="sm"><Plus size={14} /> New Article</Button></Link>
      </div>
      <Card padding={false}>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#E8E9ED]">
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-[#6B7280] uppercase">Date</th>
          ...
          <tbody>
            {articles.map((a) => (
              <tr key={a.id} className="border-b border-[#F5F6F8] hover:bg-[#F5F6F8]">
                ...
                <td className="px-4 py-3"><Badge variant={catColors[a.category]}>{a.category.replace('_', ' ')}</Badge></td>
                <td className="px-4 py-3"><Badge variant={a.status === 'PUBLISHED' ? 'success' : 'default'}>{a.status}</Badge></td>
            ...
            {articles.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-[#6B7280]">No articles yet</td></tr>}
          </tbody>
        </table>
      </Card>
    </div>
